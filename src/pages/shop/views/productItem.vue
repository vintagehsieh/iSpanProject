...
export default {
  setup() {
    ...
    const addtoCartItem = async (productId, qty) => {
      await fetch(`https://localhost:7043/Carts/AddItem/${productId}/${qty}`, {
        method: "POST",
        headers: {
    ...
    <div class="albumInfo">
      ...
      <div class="right">
        ...
        <div class="buySection">
          <div class="albumPrice">NTD$ {{ membercart.value.productPrice }}</div>
          <div class="btn">
            <input type="number" v-model="qty" min="1" step="1" />
            <button
              type="button"
              @click="addtoCartItem(membercart.value.id, qty), alertFunction()"
              class="addCartBtn"
            >
              加入購物車
            </button>
