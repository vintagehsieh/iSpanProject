...
<template>
  <div>
      ...
      <div class="row">
        <div class="col-lg-6 col-12">
          ...
          <a-form-item label="專輯名稱" name="專輯名稱">
            <a-select v-model:value="song.AlbumId" :options="prop.AlbumList"></a-select>
          </a-form-item>
          ...
          <a-form-item label="上傳歌曲檔案" name="上傳歌曲檔案">
            <a-input id="UploadSong" type="file" accept=".mp3, .wav, .ogg" @change="handleUploadSong($event)" />
          </a-form-item>
        ...
        <div class="col-lg-6 col-12">
          ...
          <a-form-item label="歌詞" name="歌詞">
            <a-textarea v-model:value="song.Lyric" :auto-size="{ minRows: 32, maxRows: 32 }" />
          </a-form-item>
    ...
    <a-button type="primary" @click="addsong" class="w-100" :loading="submit_load">Submit</a-button>
  </div>
</template>
<script setup>
...
const addsong = () => {
  ...
  axios.post(`https://localhost:7043/Creators/${prop.ID}/song`, formData, {
    withCredentials: true,
  })
    .then((res) => {
      console.log(res);
